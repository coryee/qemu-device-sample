...
void __iomem *pci_ioremap_bar_test(struct pci_dev *pdev, int bar)
{
	...
	printk(KERN_INFO "resource_size(res) =  0x%08llx\n", resource_size(res));	
...
static const struct proc_ops proc_fops = {
    .proc_open = ctc_proc_open,
    .proc_read = ctc_proc_read,
    .proc_write = ctc_proc_write,
    .proc_release = ctc_proc_release,
};
#else
...
MODULE_DESCRIPTION("just for test ctcdev driver");
MODULE_LICENSE("GPL");
